...
- **`single_peer_ip`, `wg_mgr_core`, `coord_core`: point-to-point mode and task core pinning.** For one-ESP-to-one-host setups (e.g. a sensor talking to one Home Assistant box), `single_peer_ip: 100.x.y.z` restricts the device to a single peer: only that IP gets a WireGuard slot and DISCO probes, every other tailnet node is rejected at peer-add time, and the web-UI priority-peer / max-peers settings can no longer override the restriction. It forces `max_peers` to 1 so the compiled peer table shrinks to match (RAM savings). The microlink task core pinning that previously lived only in Kconfig (`ML_WG_MGR_CORE` / `ML_COORD_CORE`) is now a YAML option too, defaulting to Core 0 — which keeps the priority-7 WireGuard/DISCO task off ESPHome's loop task (Core 1, priority 1) and stops the task-watchdog reboots that pinning both to Core 1 caused. Set `wg_mgr_core: 1` / `coord_core: 1` only in standalone (non-ESPHome) builds where Core 1 is free.
